  ...
  return (
    <ThemedView style={styles.container}>
      <View style={styles.header}>
        ...
        <ThemedText type="title" style={styles.title}>{team?.name ?? 'Команда'}</ThemedText>
      </View>
      ...
      <Animated.View style={[styles.sidebar, { width: animatedWidth }] as any} pointerEvents="box-none">
        <View style={styles.sidebarInner}>
          {isOpen ? (
            <>
              <TouchableOpacity style={styles.collapseTop} onPress={toggleSidebar}>
                <ThemedText>{'>'}</ThemedText>
              </TouchableOpacity>
  ...
  return (
    <View style={sideStyles.row}>
      ...
      <View style={sideStyles.info}>
        <ThemedText style={{ fontWeight: '700' }}>{displayName || 'Ви'}</ThemedText>
        {label ? <ThemedText style={{ color: '#666', fontSize: 12 }}>{label}</ThemedText> : null}
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  header: { height: 56, justifyContent: 'center', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: '#EEE', position: 'relative' },
  title: { textAlign: 'center' },
  back: { position: 'absolute', left: 12, top: 12 },
  content: { padding: 20 },

  // Sidebar styles
  sidebar: { position: 'absolute', right: 0, top: 0, bottom: 0, backgroundColor: '#F0F2F5', borderLeftWidth: 1, borderLeftColor: '#EEE', zIndex: 50 },
  sidebarInner: { flex: 1, paddingTop: 12 },
  sidebarToggle: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#F1F7FB', justifyContent: 'center', alignItems: 'center', alignSelf: 'center' },
