      ...
      {!budget ? (
        ...
        <View style={styles.chartArea}>
          <View style={styles.chartWrapper}>
            <View style={[styles.donut, { borderColor: currency === 'UAH' ? '#2F80ED' : currency === 'USD' ? '#27AE60' : '#9B51E0' }]}>
              <View style={styles.donutInner}>
                <Text style={styles.amountText}>{currencySymbols[budget.currency]}{formatAmount(budget.amount)}</Text>
              </View>
            </View>
          ...
          <TouchableOpacity style={styles.clearButton} onPress={() => {
            Alert.alert('Видалити бюджет', 'Ви впевнені?', [
              ...
              { text: 'Видалити', style: 'destructive', onPress: clearBudget }
            ]);
          }}>
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            <ThemedText type="title">{budget ? 'Редагувати бюджет' : 'Внести бюджет'}</ThemedText>
            ...
            <View style={styles.currencyRow}>
              {(['UAH', 'USD', 'EUR'] as const).map((c) => (
                <TouchableOpacity
                  key={c}
                  style={[styles.currencyBtn, currency === c ? styles.currencySelected : null]}
                  onPress={() => setCurrency(c)}
                >
                  <Text style={currency === c ? styles.currencyTextSelected : styles.currencyText}>{currencySymbols[c]}</Text>
                </TouchableOpacity>
              ))}
            </View>

            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.primaryButton} onPress={saveBudget}>
                <Text style={styles.primaryButtonText}>{budget ? 'Зберегти' : 'Внести'}</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  bigPlus: { fontSize: 56, color: '#fff', backgroundColor: '#007AFF', width: 120, height: 120, textAlign: 'center', lineHeight: 120, borderRadius: 60 },
  ...
  donut: { width: 220, height: 220, borderRadius: 110, borderWidth: 20, justifyContent: 'center', alignItems: 'center' },
  donutInner: { width: 140, height: 140, borderRadius: 70, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center' },
  amountText: { fontSize: 20, fontWeight: '700' },
  editButton: { position: 'absolute', right: -8, top: -8, width: 44, height: 44, borderRadius: 22, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center', elevation: 3 },
  ...
  input: { height: 48, borderWidth: 1, borderColor: '#E6EDF6', borderRadius: 8, paddingHorizontal: 12, marginTop: 8 },
  currencyRow: { flexDirection: 'row', marginTop: 8, gap: 12 },
  currencyBtn: { padding: 12, borderRadius: 8, borderWidth: 1, borderColor: '#E6EDF6', minWidth: 64, alignItems: 'center' },
